    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <Select value={target} onChange={v => { setTarget(v); setPage(1); }} options={[
          ...
          { value: 'sap_dm', label: t('logs.sapDm') },
        ...
        >
          <option value="">{t('logs.allConnections')}</option>
          {connections?.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
        </select>

        <Select value={status} onChange={v => { setStatus(v); setPage(1); }} options={[
          { value: '', label: t('logs.allStatus') },
          { value: '2xx', label: t('logs.success2xx') },
          { value: '4xx', label: t('logs.clientError4xx') },
          { value: '5xx', label: t('logs.serverError5xx') },
        ]} />
        ...
        {loading && <span className="text-xs text-gray-400 dark:text-gray-500">{t('common.loading')}</span>}
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-gray-400 dark:text-gray-500 border-b border-gray-200 dark:border-gray-700">
                ...
                <th className="px-5 py-3 font-medium">{t('common.target')}</th>
                <th className="px-5 py-3 font-medium">{t('common.method')}</th>
                <th className="px-5 py-3 font-medium">{t('common.path')}</th>
                <th className="px-5 py-3 font-medium">{t('common.status')}</th>
                <th className="px-5 py-3 font-medium">{t('common.duration')}</th>
                <th className="px-5 py-3 font-medium">{t('logs.size')}</th>
              </tr>
            </thead>
            <tbody>
              {!logs?.data.length && (
                <tr><td colSpan={7} className="px-5 py-8 text-center text-gray-400 dark:text-gray-500">{t('logs.noLogs')}</td></tr>
              )}
              {logs?.data.map(log => (
                <tr key={log.id} className="border-b border-gray-100 dark:border-gray-700/50 hover:bg-gray-50 dark:hover:bg-gray-700/30 cursor-pointer" onClick={() => setSelectedLogId(log.id)}>
                  ...
                  <td className="px-5 py-2">
                    <span className={`text-xs px-2 py-0.5 rounded-full ${
                      ...
                      {log.target === 'sap_dm' ? t('logs.sapDm') : t('logs.agent')}
                    </span>
                  </td>
        ...
        {logs && logs.pages > 1 && (
          <div className="flex items-center justify-between px-5 py-3 border-t border-gray-200 dark:border-gray-700">
            <span className="text-sm text-gray-400 dark:text-gray-500">
              {t('logs.totalPages', { total: logs.total.toLocaleString(locale), page: logs.page, pages: logs.pages })}
            </span>
